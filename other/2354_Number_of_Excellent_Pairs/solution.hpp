...
// the bitwise AND operation. Return the number of distinct excellent pairs.
...
// equal to (11) in binary. The total number of set bits is 1 + 2 = 3. So the
// number of excellent pairs is 5.
...
class Solution {
 ...
 public:
  long long countExcellentPairs(const vector<int>& nums, int k) {
    long long count = 0;
    set<int> uniq;
